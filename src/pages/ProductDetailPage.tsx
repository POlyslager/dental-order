...
export default function ProductDetailPage({ product, onBack, onUpdated, onDeleted, onAddToCart }: Props) {
  ...
  return (
    <div className="min-h-full bg-slate-50 overflow-x-hidden animate-slide-in-right">
      {/* Sub-header */}
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4"
        style={{ paddingBottom: editing ? '60vh' : '6rem' }}>
          ...
          <div className={`px-4 py-3 ${styles.bg}`}>
            ...
            {!editing && (
              <div className="flex gap-4 mt-2">
                <span className="text-xs text-slate-500">Meldebestand: {form.min_stock} {form.unit}</span>
              </div>
            )}
            {editing && (
