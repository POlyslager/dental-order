      ...
      {loading ? (
        ...
        <>
          <div className="hidden md:block bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">Kategorie</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">Artikel</th>
                  <th className="w-8" />
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {filtered.map(c => (
                  <tr key={c.name} onClick={() => openExisting(c)} className="hover:bg-slate-50 cursor-pointer transition-colors">
                    <td className="px-4 py-3.5 font-medium text-slate-800">{c.name}</td>
                    <td className="px-4 py-3.5 text-slate-400">{c.count}</td>
            ...
            {filtered.length === 0 && (
              <p className="text-sm text-slate-400 text-center py-10">Keine Kategorien gefunden</p>
            )}
          </div>

          <div className="md:hidden space-y-2">
            {filtered.map(c => (
              <div key={c.name} onClick={() => openExisting(c)}
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{c.count} Artikel</p>
                </div>
        ...
        <div className="fixed inset-0 z-30">
          ...
          <div className="absolute top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl flex flex-col animate-slide-in-right">
            ...
            <div className="border-t border-slate-100 px-5 py-4 flex gap-3 shrink-0">
              ...
              <button
                onClick={handleSave}
                disabled={saving || !newName.trim() || newName.trim() === selected.name}
