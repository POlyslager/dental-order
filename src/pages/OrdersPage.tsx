...
export default function OrdersPage({ role, user, onBadgeChange, forceOpenTab, forceScanMode, forceCartTab, forceApprovalTab }: Props) {
  ...
  const [retractedOrders, setRetractedOrders] = useState<Order[]>([])
...
const [domainToSupplier, setDomainToSupplier] = useState<Record<string, string>>({})
  ...
  useEffect(() => {
    Promise.all([fetchCart(), fetchOrders(), fetchPendingOrders(), fetchRejectedOrders(), fetchRetractedOrders()])
  }, [])
  ...
  async function fetchRetractedOrders() {
    if (role !== 'admin') return
    const since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString()
    const { data } = await supabase
      .from('orders')
      .select('id, status, total_estimate, created_at, notes')
      .eq('status', 'retracted')
      .gte('created_at', since)
      .order('created_at', { ascending: false })
      .limit(20)
    setRetractedOrders((data as unknown as Order[]) ?? [])
  }
  ...
  async function cancelPendingOnCartChange(newCartItems: CartItem[]) {
    ...
    const originalTotal = pendingOrders.reduce((s, o) => s + (o.total_estimate ?? 0), 0)
    const newTotal = newCartItems.reduce((s, i) => s + i.quantity * (effectivePrice(i.product, supplierRules[i.product?.preferred_supplier ?? '']) ?? 0), 0)
    const fmt = (n: number) => n.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })

    // Use API (service role) so RLS doesn't block the employee's update
    await fetch('/api/retract-order', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...
        new_total: newTotal,
        retracted_by: user.email ?? user.id,
      }),
    }).catch(() => null)
    ...
    fetch('/api/send-push', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title: 'Freigabe zurückgezogen',
        body: `${user.email ?? 'Mitarbeiter'} – war € ${fmt(originalTotal)}, jetzt € ${fmt(newTotal)}`,
      }),
    }).catch(() => null)

    if (newTotal < 2000) showToast('Freigabe nicht mehr erforderlich – Bestellung zurückgezogen')
    else showToast('Freigabe zurückgezogen – Warenkorb wurde geändert')
  }
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        {tab === 'approval' && role === 'admin' && (
          <div>
            ...
            {/* ── Retracted approvals history ── */}
            {retractedOrders.length > 0 && (
              <div className="border-t-2 border-slate-100 dark:border-slate-800 mt-2">
                <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 flex items-center gap-2">
                  <XCircle size={13} className="text-slate-400 shrink-0" />
                  <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Zurückgezogen (letzte 30 Tage)</span>
                </div>
                {retractedOrders.map(order => {
                  let meta: { original_total?: number; new_total?: number; retracted_by?: string; retracted_at?: string } = {}
                  try { meta = JSON.parse(order.notes ?? '{}') } catch { /* ignore */ }
                  const fmt = (n: number) => `€ ${n.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                  return (
                    <div key={order.id} className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-xs text-slate-400 dark:text-slate-500">
                          {new Date(order.created_at).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </p>
                        {meta.retracted_by && (
                          <p className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate">{meta.retracted_by}</p>
                        )}
                        {meta.original_total != null && meta.new_total != null && (
                          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                            War {fmt(meta.original_total)} → danach {fmt(meta.new_total)}
                          </p>
                        )}
                      </div>
                      <span className="shrink-0 text-xs font-medium px-2 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400">
                        Zurückgezogen
                      </span>
                    </div>
                  )
