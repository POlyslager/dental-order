  ...
  async function cancelPendingOnCartChange(newCartItems: CartItem[]) {
    ...
    await fetch('/api/retract-order', {
      ...
      body: JSON.stringify({
        ...
        original_total: pendingOrders.reduce((s, o) => s + (o.total_estimate ?? 0), 0),
        new_total: newTotal,
                          ...
                          return (
                            <React.Fragment key={domain}>
                              ...
                              {domainItems.map((item, rowIdx) => {
                                ...
                                const rowTotal = item.quantity * (price ?? 0)
                                return (
                                  <tr key={item.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border-b border-slate-100 dark:border-slate-800 ${rowIdx % 2 === 0 ? 'bg-white dark:bg-slate-900' : 'bg-white dark:bg-slate-900'}`}>
  ...
  return (
    <>
      ...
      {items.map((item, rowIdx) => {
        ...
        return (
          <tr key={item.id} className={`border-b border-slate-100 dark:border-slate-800 transition-colors ${done ? 'bg-emerald-50/60 dark:bg-emerald-900/10' : rowIdx % 2 === 0 ? 'bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800' : 'bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
